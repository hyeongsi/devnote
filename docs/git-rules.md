...
- 사용자가 정확히 `PR 요청`이라고 말한 경우, 이를 현재 변경 범위에 대한 커밋 + 푸시 + PR 생성 요청으로 간주하고 같은 규칙을 자동 적용한다.
- `PR 요청`으로 생성하는 PR은 항상 현재 변경만 담은 새 브랜치와 새 PR이어야 하며, 이미 `Merged and closed` 된 PR은 재사용하지 않는다.
- 기존 PR 브랜치에 새 커밋이 있더라도, 사용자가 정확히 `PR 요청`이라고 다시 말하면 현재 변경 범위만 기준으로 새 PR을 분리하는 것을 기본 동작으로 한다.
